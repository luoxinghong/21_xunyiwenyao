                    ...
                    with open('./症状/details/' + name_temp + '/' + name_temp + "_文章.txt", "w", encoding="utf-8") as f:
                        for p, q in enumerate(url_article_list):
                            try:
                                ...
                                html = self.parse_url(url_article)
                                content = etree.HTML(html).xpath("//div[@class='zz-articl fr f14']//text()")
...
if __name__ == '__main__':
    ...
    jibing_character_url = ["http://jib.xywy.com/html/%s.html" % i for i in 'w']
    jibing_character_name = [i for i in 'w']

    zhengzhuang_character_url = ["http://zzk.xywy.com/p/%s.html" % i for i in 'abcdefghjklmnopqrstwxyz']
    zhengzhuang_character_name = [i for i in 'abcdefghjklmnopqrstwxyz']
